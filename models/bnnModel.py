...
class DownBlock(nn.Module):
    def __init__(self, input_dim, output_dim, embedding_dim):
        ...
        self.imgFC = BayesLinear(0, 0.001, input_dim, embedding_dim)
        self.embdFC = BayesLinear(0, 0.001, embedding_dim, embedding_dim)
        self.imgOut = BayesLinear(0, 0.001, embedding_dim, output_dim)
        self.embdOut = BayesLinear(0, 0.001, embedding_dim, output_dim)
        self.outFC = BayesLinear(0, 0.001, output_dim, output_dim)
        self.act = nn.SiLU()
...
class ResidualBlock(nn.Module):
    def __init__(self, embedding_dim):
        super().__init__()
        self.fc1 = BayesLinear(0, 0.001, embedding_dim, embedding_dim)
        self.fc2 = BayesLinear(0, 0.001, embedding_dim, embedding_dim)
        self.act = nn.SiLU()
...
class AutoEncoder(nn.Module):
    def __init__(self, image_dim, embed_dim=256, condition=False, cond_dim=None):
        ...
        self.time_embed = nn.Sequential(
            BayesLinear(0, 0.001, image_dim, embed_dim),
            nn.SiLU(),
            BayesLinear(0, 0.001, embed_dim, embed_dim),
        )
        self.condition = condition
        if condition:
            self.cond_embed = nn.Sequential(
               BayesLinear(0, 0.001, cond_dim, embed_dim),
               nn.SiLU(),
               BayesLinear(0, 0.001, embed_dim, embed_dim),
            )
        ...
        self.out = BayesLinear(0, 0.001, image_dim, image_dim)

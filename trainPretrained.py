    ...
    with torch.inference_mode():
        ...
        if currValLoss < global_loss:
            ...
            torch.save({
                ...
                'epoch': e,
                'mode': 'pretrain'
            }, '/data/theodoroubp/imageGen/save/pretrained_model')
        else:
            patience += 1
            if patience == PATIENCE//5:
                break

model.load_state_dict(torch.load('/data/theodoroubp/imageGen/save/pretrained_model', map_location='cpu')['model'])
optimizer = torch.optim.Adam(model.parameters(), lr=LR)

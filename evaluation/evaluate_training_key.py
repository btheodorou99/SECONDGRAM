...
LABEL_IDX = [0, 5, 6, 7, 8, 9, 10, 11, 12]
EPOCHS = 1000
PATIENCE = 5
LR = 1e-3
BATCH_SIZE = 128
...
for run in tqdm(range(NUM_RUNS), desc=f'{key} Runs'):
    ...
    for l_idx in LABEL_IDX:
        ...
        binary = l_idx in [0, 5, 6, 7, 8, 9, 10, 11, 12]
        ...
        pickle.dump(stats, open(f'/data/theodoroubp/imageGen/stats/trainingStats_{l_idx}_{key}_{run}.pkl', 'wb'))
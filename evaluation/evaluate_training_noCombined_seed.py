    ...
    for e in tqdm(range(EPOCHS), leave=False):
        ...
        with torch.inference_mode():
            ...
            if currValLoss < global_loss:
                ...
                torch.save(model.state_dict(), f'/home/SECONDGRAM/save/downstream_model_{label}')
            else:
                patience += 1
                if patience == PATIENCE:
                    break

    model.load_state_dict(torch.load(f'/home/SECONDGRAM/save/downstream_model_{label}', map_location='cpu'))
    return model
...
for l_idx in LABEL_IDX:
    ...
    os.remove(f'/home/SECONDGRAM/save/downstream_model_AllFirst_{l_idx}_{key}_{run}')
    os.remove(f'/home/SECONDGRAM/save/downstream_model_AllPairedImputed_{l_idx}_{key}_{run}')
    os.remove(f'/home/SECONDGRAM/save/downstream_model_AllPairedSynthetic_{l_idx}_{key}_{run}')
    os.remove(f'/home/SECONDGRAM/save/downstream_model_SubsetFirst_{l_idx}_{key}_{run}')
    os.remove(f'/home/SECONDGRAM/save/downstream_model_SubsetPairedReal_{l_idx}_{key}_{run}')
    os.remove(f'/home/SECONDGRAM/save/downstream_model_SubsetPairedSynthetic_{l_idx}_{key}_{run}')

    pickle.dump(stats, open(f'/home/SECONDGRAM/stats/trainingStats_{l_idx}_{key}_{run}.pkl', 'wb'))
...
trainData = pickle.load(open('/data/theodoroubp/imageGen/data/trainData.pkl', 'rb')) + pickle.load(open('/data/theodoroubp/imageGen/data/valData.pkl', 'rb'))
...
for run in tqdm(range(NUM_RUNS), desc='GAN Runs'):
    random.seed(run)
            ...
            if i % GENERATOR_INTERVAL == 0:
                ...
                if i % SAVE_INTERVAL == 0:
                    torch.save({
                        ...
                        'epoch': e
                    }, f'/data/theodoroubp/imageGen/save/gan_model_{run}')
